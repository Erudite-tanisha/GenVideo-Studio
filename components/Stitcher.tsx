...
export const Stitcher: React.FC<StitcherProps> = ({
  ...
  useEffect(() => {
    ...
    // Filter out clips that were manually deleted AND clips already in playlist
    const newClips = initialClips.filter(
      (c) => !existingIds.has(c.id) && !deletedClipIds.has(c.id)
    );
    if (newClips.length > 0) {
      setPlaylist((prev) => [...prev, ...newClips]);
    }
  }, [initialClips, deletedClipIds]);
  ...
  const removeItem = (id: string) => {
    ...
    // Add to deleted clips tracker to prevent re-adding
    setDeletedClipIds((prev) => new Set(prev).add(id));

    if (
      currentPlayingIndex !== null &&
      playlist[currentPlayingIndex]?.id === id
    ) {
      stopSequence();
    }
  };
  ...
  useEffect(() => {
    if (currentPlayingIndex !== null && videoRef.current) {
      ...
      if (currentPlayingIndex === 0 && selectedAudioId && audioRef.current) {
        ...
        if (audioClip) {
          // Audio is already at correct speed - no processing needed!
          audioRef.current.src = audioClip.url;
          audioRef.current.currentTime = 0;
          audioRef.current.playbackRate = 1.0;  // Always 1.0
  ...
  const handleVideoEnded = () => {
    if (
      currentPlayingIndex !== null &&
      currentPlayingIndex < playlist.length - 1
    ) {
  ...
  const handleStitch = async () => {
    ...
    try {
      ...
      // if(selectedAudioId && audioRef.current){
      //   const audioClip = audioClips.find((a) => a.id === selectedAudioId);
      
      //   if (audioClip) {
      //     // Audio is already at correct speed from ElevenLabs
      //     audioRef.current.src = audioClip.url;
      ...
      //     if (!(audioRef.current as any)._connected) {
      //       const source = audioContext.createMediaElementSource(audioRef.current);
      //       source.connect(audioDestination);
      //       source.connect(audioContext.destination); // For monitoring
      //       (audioRef.current as any)._connected = true;
      //     }
      //   }
      ...
      const mediaRecorder = new MediaRecorder(videoStream, {
        mimeType: 'video/webm; codecs=vp9,opus', // opus for audio
      ...
      // if (audioRef.current) {
      //   delete (audioRef.current as any)._connected;
      // }
    }
  };
  ...
  const handleLipSync = async () => {
    ...
    const videoClip =
      playlist.find((c) => c.id === syncVideoId) ||
      initialClips.find((c) => c.id === syncVideoId);
    ...
    try {
      ...
      const newClip: VideoClip = {
        id: crypto.randomUUID(),
        url: syncedUrl,
        prompt: `Lip Sync: ${videoClip.prompt.substring(0, 30)}... + ${
          audioClip.voiceName
        }`,
        type: "subject",
      ...
      console.error(error);
      setSyncError(
        error.message || "Lip sync failed. Check API Key in environment."
      );
    } finally {
      setIsSyncing(false);
    }
  ...
  const handleExport = () => {
    ...
    if (!currentSrc) {
      alert(
        "No video loaded to export. Please select a clip from the sequence or stitch your clips."
      );
      return;
    }
      ...
      <div className="lg:col-span-3 flex flex-col bg-slate-900 rounded-xl border border-slate-800 overflow-hidden">
            ...
            playlist.map((clip, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <p
                    className="text-[10px] text-slate-300 line-clamp-2"
                    title={clip.prompt}
                  >
                    {clip.prompt}
                  </p>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 grid grid-cols-2 gap-2">
          <Button
            ...
            disabled={
              playlist.length === 0 ||
              (currentPlayingIndex !== null && isPlaying) ||
              isStitching
            }
          >
      ...
      <div className="lg:col-span-4 bg-slate-900/50 rounded-xl border-2 border-blue-500/40 shadow-[0_0_20px_rgba(59,130,246,0.15)] p-5 flex flex-col overflow-y-auto relative">
        ...
        <div className="bg-blue-950/20 rounded-lg p-3 mb-6 border border-blue-900/30">
          <p className="text-xs text-blue-100/80 leading-relaxed flex gap-2">
            ...
            Select a video clip and a voice clip. AI will animate the
            character's mouth to match the speech perfectly.
          </p>
        </div>
        ...
        <div className="space-y-6 flex-1">
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-200 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-xs text-blue-400 border border-slate-700">
                1
              </span>
              Select Video Clip
            </label>
          ...
          <div className="space-y-2">
            <label className="text-sm font-semibold text-slate-200 flex items-center gap-2">
              <span className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-xs text-purple-400 border border-slate-700">
                2
              </span>
              Select Audio Track
            </label>
            ...
            {isStitching && (
              <div className="absolute inset-0 z-20 bg-black/80 flex flex-col items-center justify-center text-white">
                ...
                <p className="text-sm text-slate-400 mt-2">
                  Processing frame by frame: {stitchProgress}%
                </p>
              </div>
            )}
        ...
        <div className="bg-slate-900 rounded-xl p-4 border border-slate-800 flex justify-between items-center shadow-lg">
          <div>
            ...
            <p className="text-xs text-slate-400">
              Resolution: High Quality • Total Clips: {playlist.length} • Audio:{" "}
              {selectedAudioId ? "Yes" : "No"}
            </p>
          </div>
          <Button
            variant="secondary"
